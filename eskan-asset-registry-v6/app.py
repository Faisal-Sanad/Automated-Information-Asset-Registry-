...
def apply_findings():
    ...
    for finding_id in accepted_ids:
        try:
            cols, rows = db.fetch_all(
                "SELECT asset_id, finding_type, csv_value, register_value, detail FROM discrepancies WHERE id = %s::uuid",
                (finding_id,)
            )
            if not rows:
                skipped += 1
            ...
            asset_id, finding_type, csv_value, register_value, detail = rows[0]
            ...
            if finding_type == 'Misclassified':
                ...
                if match:
                    ...
                    if csv_value and csv_value.strip():
                        ...
                        errors.append(f'Finding {finding_id}: no value to apply for missing field "{field}"')
                        skipped += 1
                        continue
                else:
                    skipped += 1
...
def reconcile():
    ...
    path = f'uploaded_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
    f.save(path)
    try:
        run_id, findings = run_reconciliation(path, run_by=request.current_user)
        return jsonify({
            'run_id': str(run_id),
...
def import_template():
    ...
    writer.writerow([
        'AR-CBS-001', 'Core Banking System', 'Customer Data',
        ...
        'High', 'Critical', 'High', 'Unauthorised access, data leakage'
    ])
    output.seek(0)
...
def approve_change(change_id):
    ...
    if action == 'INSERT':
        ...
        # Must match the column structure of add_asset (admin path) exactly.
