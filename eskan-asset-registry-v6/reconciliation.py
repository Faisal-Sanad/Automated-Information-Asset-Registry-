...
def save_run(csv_filename, total_csv, full_match, undocumented, misclassified, missing_fields, mixed, findings, run_by):
    ...
    for f in findings:
        db.execute("""
            INSERT INTO discrepancies (run_id, asset_id, finding_type, detail, csv_value, register_value)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (run_id, f["asset_id"], f["finding_type"], f["detail"], f["csv_value"], f["register_value"]))
